% two wheel cornering sim
%this function determines max velocity the tire can give

function f = calculateCornerV_new(tyre_model,mass,air_density,frontel_area,CLc,curv,camber,slip_ang,max_rpm,FDR,R,tc_lat,sen_lat,phit,P,useMode)

max_speed = (max_rpm/FDR)*pi*2*R/60;
P = convpres(P, 'psi', 'Pa'); % convert psi to pa


f = [9,0.05,0.2,0.05,0.2,0.2];
...
lb = [0,0,0,0,0,0];
ub = [max_speed,0.2,f(6),0.2,f(6),max_steer];
