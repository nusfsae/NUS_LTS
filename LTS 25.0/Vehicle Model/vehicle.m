...
beta = prob.variable(); prob.subject_to(-deg2rad(10)<=beta<=deg2rad(10));   % body slip (rad)
dpsi = prob.variable(); prob.subject_to(-deg2rad(120)<=dpsi<=deg2rad(120));   % yaw rate (rad/s)
Sxf = prob.variable(); prob.subject_to(-0.1<=Sxf<=0.1);                    % front slip ratio
Sxr = prob.variable(); prob.subject_to(-0.1<=Sxr<=0.1);                    % rear slip ratio
...
if V<22.2  %24.03 % 360V setting
    ...
    tractive = 0.8*Ipeak*Iweak*FDR/R-Drag;
end
