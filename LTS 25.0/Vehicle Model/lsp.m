function final_lsp = lsp(C2,dist,camber,tyre_model,mass,air_density,frontel_area,CLs,CLc,CDs,CDc,Accel_LSP)
...
for point = 1:count-1 %for every interval between 2 local minima
    ...
    for i = 0:local_min(point,2) %for every point in the interval
        ...
        %disp(target);%target is correct
        distance = dist(target)-dist(target-1);%distance between previous point and the local minimum

        v1 = final_lsp(target-1);
        v2 = final_lsp(target);         

        SA = slip_ang(target-1);
            
        braked_v = v_finder(v1,v2,distance,camber,SA,tyre_model,mass,air_density,frontel_area,CL,CD); %velocity after braking with these conditions
