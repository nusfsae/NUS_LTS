...
for nc=1:size(map,1)
    ...
    plot3(x(iv),y(iv),v(iv),marker,'markersize',10,'color',map(nc,:),'markerfacecolor',map(nc,:))
end
minimum = v == miv;
plot3(x(minimum),y(minimum),v(minimum),marker,'markersize',10,'color',map(1,:),'markerfacecolor',map(1,:));
hold off

% Re-format the colorbar
h = colorbar;
s=char(11,4);
ytl=linspace(0,mav,11);
for i=1:11
    if min(abs(ytl)) >= 0
