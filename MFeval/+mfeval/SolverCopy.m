classdef SolverCopy
    ...
    methods
        ...
        function out = fullSteadyState(obj, tirParams, inputs, useMode)%#codegen
            ...
            out(:,1) = real(forcesAndmoments.Fx0);
            out(:,2) = real(forcesAndmoments.Fy0);
        ...
        function out = fullCPI(obj, tirParams, inputs, userUseMode, userDynamics)%#codegen
            ...
            [~, Romega, omega] = obj.calculateRe(tirParams, postProInputs, dpi);
            [rho, Rl, ~] = obj.calculateRhoRl(tirParams, postProInputs, forcesAndmoments, dpi, omega, Romega);
            ...
            % Pack all the outputs
            out(:,1) = forcesAndmoments.Fx0;
            out(:,2) = forcesAndmoments.Fy0;
    ...
    methods (Hidden)
        ...
        function [Fy0, muy, Kya, Kyg0, SHy, SVy, By, Cy, internalParams] = calculateFy0(~, tirParams, postProInputs, internalParams, modes, starVar, primeVar, incrVar)%#codegen
            ...
            % Check MF version
            if tirParams.FITTYP == 6 || tirParams.FITTYP == 21
        ...
        function [Mx] = calculateMx(~, tirParams, postProInputs, incrVar, Fy)%#codegen
            ...
            if any([QSX12, QSX13, QSX14]) && any([QSX1, QSX2, QSX3, QSX4, QSX5, QSX6, QSX7, QSX8, QSX9, QSX10, QSX11])
                ...
                warning('Solver:Mx:AlternativeEquation','The parameter sets QSX1 to QSX11 and QSX12 to QSX14 cannnot be both non-zero')
                ...
                Mx = R0.* Fz.*LMX.*(QSX1.*LVMX-QSX2.*gamma.*(1+PPMX1.*dpi)+QSX3.*(Fy./Fz0)...
                    + QSX4.*cos(QSX5.* atan((QSX6.*(Fz./Fz0)).^2)).*sin(QSX7.*gamma+QSX8.*atan(QSX9.*(Fy./Fz0)))...
        ...
        function [Mz,t, Mzr] = calculateMz(obj, tirParams, postProInputs, internalParams, modes, starVar, primeVar, incrVar, alphar, alphat, Kxk, Kya_prime, Fy, Fx, Dr, Cr, Br, Dt, Ct, Bt, Et, SVyk)%#codegen
            ...
            % Evaluate Fy0 with gamma = 0 and phit  = 0
            [Fy0_sub0, muy_sub0, ~, ~, ~, ~, ~, ~] = obj.calculateFy0(tirParams, postProInputs_sub0, internalParams_sub0, modes, starVar_sub0, primeVar, incrVar);
            
            % Evaluate Gyk with phit = 0 (Note: needs to take gamma into
            % account to match TNO)
            [~,Gyk_sub0] = obj.calculateFy(tirParams, postProInputs, internalParams_sub0, modes, starVar, incrVar, Fy0_sub0, muy_sub0);
        ...
        function [rho, Rl, Cz] = calculateRhoRl(obj, tirParams, postProInputs, forcesAndmoments, dpi, omega, Romega)%#codegen
            
            % Calculate the radius depending on the MF version
            if tirParams.FITTYP == 62
                % MF6.2
        ...
        function [rho_z, Rl, Cz] = calculateRhoRl62(obj, tirParams, postProInputs, forcesAndmoments, dpi, omega, Romega)%#codegen
            ...
            R0 = ones(size(omega))*tirParams.UNLOADED_RADIUS;
            Fz_tol = tirParams.FNOMIN * 1e-6;
            
            % Cost function to optimize
            fun = @(try_radius) obj.delta_fz_func(tirParams, gamma, omega, Romega, dpi, try_radius, Fx, Fy, Fz);
            
            % Solve with the secant method
            Rl = obj.solve_secant(fun, R0*0.95, R0, Fz_tol);
            
            % Calculate the deflection
            [~, rho_z] = obj.calculateFz62(tirParams, gamma, omega, Romega, dpi, Rl, Fx, Fy);
    ...
    methods (Access = protected)
        ...
        function [forcesAndmoments, varinf] = doForcesAndMoments(obj, tirParams, postProInputs, internalParams, modes)%#codegen
            ...
            [Fy, ~, SVyk] = obj.calculateFy(tirParams, postProInputs, internalParams, modes, starVar, incrVar, Fy0, muy);
            
            [Mx] = obj.calculateMx(tirParams, postProInputs, incrVar, Fy);
            
            [My] = obj.calculateMy(tirParams, postProInputs, incrVar, Fx);
            
            [Mz, t, Mzr] = obj.calculateMz(tirParams, postProInputs, internalParams, modes, starVar, primeVar, incrVar, alphar, alphat, Kxk, Kya_prime, Fy, Fx, Dr, Cr, Br, Dt, Ct, Bt, Et, SVyk);
            
            % Pack outputs
            forcesAndmoments.Fx0 = Fx0;
            forcesAndmoments.Fy0 = Fy0;
        ...
        function [Re, omega, rho, Rl, a, b, Cx, Cy, Cz, sigmax, sigmay, instKya] = doExtras(obj, tirParams, postProInputs, forcesAndmoments, varinf)%#codegen
            ...
            [rho, Rl, Cz] = obj.calculateRhoRl(tirParams, postProInputs, forcesAndmoments, dpi, omega, Romega);
        ...
        function x = solve_secant(~, fun, x0, x1, ytol)
            ...
            for calls = 3:20 % to limit number of tries
        ...
        function delta_fz = delta_fz_func(obj, tirParams, gamma, omega, Romega, dpi, try_radius, Fx, Fy, Fz)
            [try_Fz, ~] = obj.calculateFz62(tirParams, gamma, omega, Romega, dpi, try_radius, Fx, Fy);
            delta_fz = try_Fz - Fz;
        end % delta_fz_func
