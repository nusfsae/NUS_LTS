...
% p (tire pressure)
function [Fx, Fy] = MFrelation(tirParams, Fz, kappa, alpha, gamma, Vcx, p)
    ...
    % Create a copy of the variables to be used if useLimitsCheck == false (u stands for unlimited)
    uFz = Fz;
    ukappa = kappa;
    ukappaLow = kappa;
    ualpha = alpha;
    ...
    [Fx0, mux] = pureLong(Fz, kappa, gamma, LMUX_star, LMUX_prime, isLowSpeed, reductionSmooth, epsilonx);
    [Fx] = combLong(Fx0, gamma_star, kappa, alpha_star);
    [Fy] = combLat(Fz0, Fz, Fx, alpha, alpha_star, gamma_star, LMUY_star, LMUY_prime, epsilonk, epsilony, epsilonx,  mux, reductionSmooth, isLowSpeed);


    % Fx0 pure longitudinal slip
    function [Fx0, mux] = pureLong(Fz, kappa, gamma, LMUX_star, LMUX_prime, isLowSpeed, reductionSmooth, epsilonx)
    ...
    function [Fx] = combLong(Fx0, gamma_star, kappa, alpha_star)
        %[SCALING_COEFFICIENTS]
     ...
     function [Fy0,Kya,Kyg0] = pureLat(Fz, alpha_star, gamma_star, LMUY_star, LMUY_prime, epsilonk, epsilony, reductionSmooth, isLowSpeed) 
        %[SCALING_COEFFICIENTS]
    ...
    function [Fy] = combLat(Fz0, Fz, Fx, alpha, alpha_star, gamma_star, LMUY_star, LMUY_prime, epsilonk, epsilony, epsilonx, mux, reductionSmooth, isLowSpeed)
        ...
        Cfa = phixa * (Kya - 0.5*mux*Fz) + 0.5*(mux*Fz - Fx); % (4.42 page 164)
        Cfg = phix^2 * Kyg0; % (4.43 page 164)

        alpha_eq = 1/phix * (Cfa/Kya)*(mux0*Fz0)/(mux*Fz)*(alpha+(Cfg/Cfa)); % (4.46 page 164)
        [Fy0eq, ~, ~] = pureLat(Fz0, alpha_eq, gamma_star, LMUY_star, LMUY_prime, epsilonk, epsilony, reductionSmooth, isLowSpeed);
