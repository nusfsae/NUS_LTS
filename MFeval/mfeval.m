...
%
% For example, generate a plot of slip angle vs lateral force:
%   Fz = ones(100, 1)*6000;
%   kappa = zeros(100, 1);
...
%   phit = zeros(100, 1);
%   Vx = ones(100, 1)*16;
%   out = mfeval('MagicFormula61_Parameters.tir', [Fz kappa alpha gamma phit Vx], 111);
%   plot(alpha, out(:,2))
%
...
assert(isequal(mfStruct.FITTYP,62) || isequal(mfStruct.FITTYP,61) || isequal(mfStruct.FITTYP,6) || isequal(mfStruct.FITTYP,21), ...
    'MFeval:parameterSource:NotValid', 'The input parameterSource is not compatible with Magic Formula 5.2, 6.1 or 6.2 \nThe coefficient FITTYP in the tyre property file (.tir) should be one of the followings:\n FITTYP = 6  - MF5.2\n FITTYP = 21 - MF5.2\n FITTYP = 61 - MF6.1\n FITTYP = 62 - MF6.2');

% Get the solver
SolverCopy = mfeval.SolverCopy;

% Run solver
[outMF] =  SolverCopy.fullSteadyState(mfStruct, inputsMF, useMode);
